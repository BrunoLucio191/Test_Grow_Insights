...
import type { CacheStatus, ClientRow } from "../lib/analytics-types.ts";
import { scopeKey, isPlaceholder } from "@/lib/utils.ts";
import { EMPTY_PAID, EMPTY_ORGANIC } from "@/constantes/metaDefaults.ts";
  ...
  .handler(async ({ data }): Promise<CacheStatus> => {
    ...
    const build = (scope: string) => {
      ...
      const expiresAt = new Date(
        new Date(fetchedAt).getTime() + CACHE_DURATION_IN_SECONDS * 1000,
      ).toISOString();
    ...
    return {
      ...
      ttlSeconds: CACHE_DURATION_IN_SECONDS,
    };
  });
